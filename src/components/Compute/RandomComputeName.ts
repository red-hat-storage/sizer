...
// Copied shamelessly from docker container name generator
const left = [
  "admiring",
  "adoring",
  "affectionate",
...
export const getRandomName = (): string => {
  const leftRandomNumber = _.random(0, left.length - 1);
  const rightRandomNumber = _.random(0, right.length - 1);

  return `${left[leftRandomNumber]}-${right[rightRandomNumber]}`;
};
